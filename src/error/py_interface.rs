...
use super::indicator_error::IndicatorError; // 导入 IndicatorError
...
create_exception!(pyo3_quant.errors, PySourceIndexOutOfBoundsError, PySignalError);
...
create_exception!(pyo3_quant.errors, PyIndicatorParameterNotFoundError, PyIndicatorError);
create_exception!(pyo3_quant.errors, PyIndicatorInvalidParameterError, PyIndicatorError);
create_exception!(pyo3_quant.errors, PyIndicatorColumnNotFoundError, PyIndicatorError);
create_exception!(pyo3_quant.errors, PyIndicatorDataTooShortError, PyIndicatorError);
...
fn convert_indicator_error(indicator_error: IndicatorError) -> PyErr {
    match indicator_error {
        IndicatorError::ParameterNotFound(param, indicator) => PyIndicatorParameterNotFoundError::new_err(format!("Parameter '{}' not found for indicator '{}'", param, indicator)),
        IndicatorError::InvalidParameter(param, value) => PyIndicatorInvalidParameterError::new_err(format!("Invalid parameter for '{0}': {1}", param, value)),
        IndicatorError::ColumnNotFound(col) => PyIndicatorColumnNotFoundError::new_err(format!("Input column '{0}' not found", col)),
        IndicatorError::DataTooShort(indicator, period) => PyIndicatorDataTooShortError::new_err(format!("Input data is too short to calculate indicator '{0}' with period '{1}'", indicator, period)),
    }
}
...
pub fn register_py_module(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add("PySourceNotFoundError", PySourceNotFoundError::type_object(m.py()))?;
    m.add("PySourceIndexOutOfBoundsError", PySourceIndexOutOfBoundsError::type_object(m.py()))?;
    m.add("PyColumnNotFoundError", PyColumnNotFoundError::type_object(m.py()))?;
    m.add("PyInvalidSourceFormatError", PyInvalidSourceFormatError::type_object(m.py()))?;
    m.add("PyMappingColumnNotFound", PyMappingColumnNotFound::type_object(m.py()))?;
    m.add("PyMappingCastError", PyMappingCastError::type_object(m.py()))?;
    m.add("PyMappingApplyError", PyMappingApplyError::type_object(m.py()))?;
    m.add("PyParameterNotFoundError", PyParameterNotFoundError::type_object(m.py()))?;

    // 注册 IndicatorError 相关的异常
    m.add("PyIndicatorParameterNotFoundError", PyIndicatorParameterNotFoundError::type_object(m.py()))?;
    m.add("PyIndicatorInvalidParameterError", PyIndicatorInvalidParameterError::type_object(m.py()))?;
    m.add("PyIndicatorColumnNotFoundError", PyIndicatorColumnNotFoundError::type_object(m.py()))?;
    m.add("PyIndicatorDataTooShortError", PyIndicatorDataTooShortError::type_object(m.py()))?;
