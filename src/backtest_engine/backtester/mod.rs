...
pub fn run_backtest(
    ...
    let prepared_data = prepare_data(processed_data, &signals_df_after_mask, &atr_series)?;
    ...
    // 4. 运行回测主循环
    let mut output_buffers = run_main_loop(
        &prepared_data,
        &mut backtest_state,
        output_buffers,
        backtest_params,
    ...
    output_buffers.to_dataframe().map_err(|e| e.into())
}
