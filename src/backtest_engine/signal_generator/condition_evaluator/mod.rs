...
use comparison_eval::{
    perform_crossover_comparison, perform_range_comparison, perform_simple_comparison,
};
use polars::prelude::*;
use std::ops::{BitAnd, BitOr, Not};

fn resolved_operand_len(operand: &ResolvedOperand) -> usize {
    match operand {
        ResolvedOperand::Series(series_vec) => series_vec.len(),
        ResolvedOperand::Scalar(_) => 1,
    }
}

fn build_zone_broadcast_triplets(
    left_len: usize,
    right_len: usize,
    end_len: usize,
    condition: &SignalCondition,
) -> Result<Vec<(usize, usize, usize)>, QuantError> {
    let max_len = left_len.max(right_len).max(end_len);
    let lens = [left_len, right_len, end_len];
    let has_invalid_len = lens.iter().any(|&len| len != 1 && len != max_len);
    ...
    if has_invalid_len {
        return Err(QuantError::Signal(SignalError::InvalidOffset(format!(
            "区间穿越操作数长度不匹配，无法进行广播。\n\
             ...
             左操作数: {:?}\n\
             右操作数: {:?}\n\
             提示：广播规则为 (1,1,1), (n,1,1), (1,n,1), (1,1,n), (n,n,1), (n,1,n), (1,n,n), (n,n,n)",
            left_len, right_len, end_len, condition.left, condition.right
        ))));
    }

    Ok((0..max_len)
        .map(|idx| {
            let left_idx = if left_len == 1 { 0 } else { idx };
            let right_idx = if right_len == 1 { 0 } else { idx };
            let end_idx = if end_len == 1 { 0 } else { idx };
            (left_idx, right_idx, end_idx)
        })
        .collect())
}
...
pub fn evaluate_parsed_condition(
    ...
    let zone_end_resolved = if let Some(zone_end_operand) = &condition.zone_end {
        Some(resolve_right_operand(
            zone_end_operand,
            processed_data,
            indicator_dfs,
            signal_params,
    ...
    if let Some(end_resolved) = &zone_end_resolved {
        let end_len = resolved_operand_len(end_resolved);
        let comparison_triplets =
            build_zone_broadcast_triplets(left_len, right_len, end_len, condition)?;

        for (left_idx, right_idx, end_idx) in comparison_triplets {
            let left_s = &left_series_vec[left_idx];
            let (comparison_result, mask_result) = perform_range_comparison(
                left_s,
                &right_resolved,
                right_idx,
                end_resolved,
        ...
        for (left_idx, right_idx) in comparison_pairs {
            ...
            let (comparison_result, mask_result) = match condition.op {
                ...
                CompareOp::XGT => perform_crossover_comparison(
                    left_s,
                    &right_resolved,
                    right_idx,
                    |a, b| a.gt(b),
                ...
                CompareOp::XLT => perform_crossover_comparison(
                    left_s,
                    &right_resolved,
                    right_idx,
                    |a, b| a.lt(b),
                ...
                CompareOp::XGE => perform_crossover_comparison(
                    left_s,
                    &right_resolved,
                    right_idx,
                    |a, b| a.gt_eq(b),
                ...
                CompareOp::XLE => perform_crossover_comparison(
                    left_s,
                    &right_resolved,
                    right_idx,
                    |a, b| a.lt_eq(b),
                ...
                CompareOp::XEQ => perform_crossover_comparison(
                    left_s,
                    &right_resolved,
                    right_idx,
                    |a, b| a.equal(b),
                ...
                CompareOp::XNE => perform_crossover_comparison(
                    left_s,
                    &right_resolved,
                    right_idx,
                    |a, b| a.not_equal(b),
                ...
                CompareOp::IN | CompareOp::XIN => {
                    return Err(QuantError::Signal(SignalError::InvalidInput(
                        "in/xin 必须与 .. 一起使用".to_string(),
                    )));
                }
            };
