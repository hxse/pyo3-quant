...
class TestLongShortSymmetry:
    ...
    def test_risk_triggers_distribution(self, backtest_df):
        """测试多空风控触发分布"""
        long_risk_count = len(
            backtest_df.filter(pl.col("risk_exit_long_price").is_not_nan())
        )
        short_risk_count = len(
            backtest_df.filter(pl.col("risk_exit_short_price").is_not_nan())
        )

        print(f"📊 风控触发统计:")
        print(f"  - 多头风控: {long_risk_count}次")
        print(f"  - 空头风控: {short_risk_count}次")
        print("✅ 多空风控触发统计完成")

    def test_sl_tp_logic_consistency(self, backtest_df):
        """测试止损止盈逻辑一致性"""
        if "sl_pct_price" not in backtest_df.columns:
            pytest.skip("未启用百分比止损")

        # 检查 in_bar 风控触发
        in_bar_exits = backtest_df.filter(pl.col("risk_exit_in_bar") == True)

        print(f"📊 In-Bar 风控触发: {len(in_bar_exits)}次")

        if len(in_bar_exits) > 0:
            # in_bar 触发时应该有对应的 exit_price
            long_in_bar = in_bar_exits.filter(
                pl.col("risk_exit_long_price").is_not_nan()
            )
            short_in_bar = in_bar_exits.filter(
    ...
    def test_risk_price_sanity(self, backtest_df):
        ...
        long_positions = backtest_df.filter(
            ...
            & pl.col("sl_pct_price").is_not_nan()
        )

        if len(long_positions) > 0:
            # 止损价应该低于进场价
            invalid_sl = long_positions.filter(
                pl.col("sl_pct_price") >= pl.col("entry_long_price")
            )
