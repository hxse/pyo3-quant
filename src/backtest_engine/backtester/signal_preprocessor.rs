...
mod tests {
    ...
    #[test]
    fn test_has_leading_nan_no_longer_suppresses_entry() {
        let signals = DataFrame::new(vec![
            Series::new(ColumnName::EntryLong.as_pl_small_str(), vec![true]).into(),
            Series::new(ColumnName::ExitLong.as_pl_small_str(), vec![false]).into(),
            Series::new(ColumnName::EntryShort.as_pl_small_str(), vec![false]).into(),
            Series::new(ColumnName::ExitShort.as_pl_small_str(), vec![false]).into(),
            Series::new(ColumnName::HasLeadingNan.as_pl_small_str(), vec![true]).into(),
        ])
        .expect("signals df 应成功");

        let processed = preprocess_signals(signals, None, &None).expect("预处理应成功");
        let entry_long = processed
            .column(ColumnName::EntryLong.as_str())
            .expect("entry_long")
            .bool()
            .expect("bool")
            .get(0)
