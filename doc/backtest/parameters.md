...
## 4. 暂停机制 (Pause Mechanics)

当市场环境不利时，可以通过以下参数暂停新开仓（Pause New Entries）。参数值为阈值，`<= 0.0` 表示不启用。

> [!WARNING]
> **互斥约束**：以下三个暂停参数中，**最多只能启用一个**。

| 参数名 | 说明 |
|--------|------|
| `pause_drawdown` | **回撤暂停**。当账户净值从历史高点回撤超过此百分比时，暂停开仓。 |
| `pause_sma` | **均线暂停 (SMA)**。当净值低于净值的 SMA 均线时，暂停开仓。 |
| `pause_ema` | **均线暂停 (EMA)**。当净值低于净值的 EMA 均线时，暂停开仓。 |
