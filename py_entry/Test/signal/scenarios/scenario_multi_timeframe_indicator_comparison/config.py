...
INDICATORS_PARAMS = {
    "ohlcv_15m": {
        # 短期趋势指标
        "ema_0": {"period": Param(21, min=10, max=50, step=5)},  # 15分钟EMA
        # 短期动量指标
        "rsi_0": {"period": Param(14, min=5, max=30, step=1)},
        # 短期波动率指标
        "bbands_0": {
            "period": Param(20, min=10, max=30, step=5),
            "std": Param(2.0, min=1.5, max=3.0, step=0.5),
        },
    },
    "ohlcv_1h": {
        # 中期趋势指标
        "ema_0": {"period": Param(21, min=10, max=50, step=5)},  # 1小时EMA
        # 中期动量指标
        "rsi_0": {"period": Param(14, min=5, max=30, step=1)},
        # 中期波动率指标
        "bbands_0": {
    ...
    "ohlcv_4h": {
        # 长期趋势指标
        "ema_0": {"period": Param(21, min=10, max=50, step=5)},  # 4小时EMA
        # 长期动量指标
        "rsi_0": {"period": Param(14, min=5, max=30, step=1)},
    },
}
...
SIGNAL_PARAMS = {
    # RSI阈值
    "rsi_midline": Param(50.0, min=40.0, max=60.0, step=5.0),
}
