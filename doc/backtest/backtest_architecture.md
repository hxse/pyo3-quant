...
```mermaid
flowchart TD
    A[更新 SL/TP/TSL 价格阈值] --> B{检查触发条件}
    B -- SL 触发 --> C[计算离场价格]
    B -- TP 触发 --> C
    B -- TSL 触发 --> C
    C --> D{exit_in_bar?}
    D -- true --> E[In-Bar 离场]
    D -- false --> F[标记 Next-Bar 离场]
```
...
| 模式 | 触发时机 | 离场价格 | 状态标记 |
|-----|---------|---------|---------|
| **In-Bar** | 当前 Bar 内 | 触发价（SL/TP/TSL 价格） | `in_bar_direction = 1/-1` |
| **Next-Bar** | 下一 Bar 开盘 | `next_bar.open` | `in_bar_direction = 0` |

#### `in_bar_direction` 字段
- **类型**: `i8`
- **含义**: `0` = 无 in-bar 离场, `1` = 多头 in-bar 离场, `-1` = 空头 in-bar 离场
...
> [!TIP]
> **风控离场推断**：
> 已移除冗余的 `risk_exit_price` 列。用户通过 `exit_long_price` 且 `risk_in_bar_direction == 1`（或 short 对应 -1）即可识别风控离场点。
