...
from py_entry.runner.results._export_pipeline import (
    copy_data_pack_for_export,
    copy_result_pack_for_export,
)
...
def _snapshot_columns(runner_with_results) -> dict[str, object]:
    ...
    data_source_cols = {
        ...
        for key, df in runner_with_results.session.data_pack.source.items()
    }
    indicator_cols = (
        {
            key: tuple(df.columns)
            for key, df in (runner_with_results.raw.indicators or {}).items()
        }
        if runner_with_results.raw.indicators is not None
        else None
    )
    signal_cols = (
        tuple(runner_with_results.raw.signals.columns)
        if runner_with_results.raw.signals is not None
        else None
    )
    backtest_cols = (
        tuple(runner_with_results.raw.backtest_result.columns)
        if runner_with_results.raw.backtest_result is not None
        else None
    )
    return {
...
def test_prepare_export_does_not_mutate_original_dataframes(runner_with_results):
    """prepare_export 必须只操作导出副本，不得修改原始计算态 DataFrame 列结构。"""
    before = _snapshot_columns(runner_with_results)

    runner_with_results.prepare_export(FormatResultsConfig(dataframe_format="csv"))

    after = _snapshot_columns(runner_with_results)
    assert before == after, "prepare_export 不应污染原始 data_pack/result 列结构"


def test_prepare_export_still_generates_export_artifacts(runner_with_results):
    """副本导出模式下仍应正常产出导出缓存和图表配置。"""
    bundle = runner_with_results.prepare_export(
        FormatResultsConfig(dataframe_format="csv")
    )
    assert bundle.buffers
    assert bundle.zip_buffer is not None
    assert bundle.chart_config is not None
...
def test_copy_data_pack_returns_independent_dataframe_fields(runner_with_results):
    ...
    original = runner_with_results.session.data_pack
    copied = copy_data_pack_for_export(original)

    copied_mapping = copied.mapping.with_columns(pl.lit(1).alias("__copy_only_mapping"))
    assert "__copy_only_mapping" in copied_mapping.columns
    assert "__copy_only_mapping" not in original.mapping.columns

    if copied.skip_mask is not None:
        copied_skip_mask = copied.skip_mask.with_columns(
            pl.lit(False).alias("__copy_only_skip")
        )
        assert "__copy_only_skip" in copied_skip_mask.columns
...
def test_copy_result_pack_returns_independent_payload_fields(runner_with_results):
    ...
    original = runner_with_results.raw
    copied = copy_result_pack_for_export(
        runner_with_results.session.data_pack, original
    )
    ...
    copied_signals = copied.signals.with_columns(pl.lit(1).alias("__copy_only_signal"))
    assert "__copy_only_signal" in copied_signals.columns
    ...
    copied_backtest = copied.backtest_result.with_columns(
        pl.lit(1.0).alias("__copy_only_backtest")
    )
    assert "__copy_only_backtest" in copied_backtest.columns
