...
*   **可运行不等于正确**：严禁以“代码能跑”作为正确性标准；必须先定义口径、边界条件与返回契约，再验证实现是否严格满足。
*   **方案驱动优先**：复杂链路（如回测/优化/WF/拼接）必须先完成文档化设计与关键算法审查，再进入实现与测试阶段，避免潜在隐患难以追溯。
*   **文档先行审查**：默认先提交可审阅文档（架构、关键算法、输入输出契约、失败策略、边界问题），由用户先审文档；文档通过后再改代码。
*   **一致性验收**：实现完成后的验收重点是“实现是否与文档一致”，而不是要求用户逐行审阅源代码。
*   **Plan 的灵魂地位**：
