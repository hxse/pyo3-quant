...
6. **严格的基础分类前缀校验**：系统底层会在回测引擎层截取指标键名的第一个 `_` 之前的部分作为**分类基础名**（如 `macd_base` 截取得到 `macd`），进而向系统注册表查找计算算子。**如果用户瞎写基础名前缀**（例如把 `macd_base` 写成 `mymacd_base`），由于系统注册表中不存在 `mymacd`，引擎管线将在启动初期直接抛出 `IndicatorError::NotImplemented("Indicator 'mymacd' is not supported")` 导致程序完全崩溃中断，因此基础分类前缀必须严格按规范书写，不能存在二次重写或魔改基础名的情况。

7. **严格的输出组件后缀校验**：对于多输出指标，Rust 底层指标算子（如 `macd::indicator::MacdIndicator`）硬编码拼接绑定了规范后缀（如强制通过 `format!("{}_macd", indicator_key)` 等成对输出）。如果你在信号判断或配置处写错了后缀名（例如你想用 MACD 信号线，却写成 `macd_0_sig` 而不是规范的 `macd_0_signal`），底层信号解析引擎（`try_resolve_series`）在从指标 DataFrame 拉取列数据时，会直接抛出 `SignalError::ColumnNotFound("macd_0_sig")`，回测管线将被立刻熔断。因此前后缀都牢牢绑定在引擎规则上，只有中间的自定义标识符能自主变动。

8. **实现原理**：详见文末的"实现细节"章节
