...
def generate_time_mapping(
    ...
    """
    生成时间映射,将base_data_key对应DataFrame的time映射到所有其他DataFrame的索引

    Args:
        source: 数据源字典
        base_data_key: 基准数据的键
    ...
    """
    if not source or base_data_key not in source:
        return pl.DataFrame(), {}

    base_df = source[base_data_key]
    skip_mapping = {}

    # 基准索引，默认就是从0开始的自然数列，必然会被跳过
    skip_mapping[base_data_key] = True
    ...
    for key, df in source.items():
        ...
        result = process_dataframe_mapping(base_df, df, key, result, skip_mapping)
