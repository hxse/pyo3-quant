...
> - 1小时：(Diff - Dea) `<` 0 (青柱) `AND` close `<` EMA
> - 日线：CCI `<` -30 `AND` close `<` EMA
> - 周线：CCI `<` -80 `AND` close `<` EMA
...
- **5分钟**：**刚刚触发** (`MACD 红柱` 且 `close > EMA`)。
    - 触发公式: `(MACD > 0 AND close x> EMA) OR (close > EMA AND MACD x> 0)`
...
- **1小时**：动能转多 (`MACD 红柱`)。
- **5分钟**：**刚刚触发** (`MACD 红柱` 且 `close > 5m EMA`) 且 (`1h EMA < close < 日线 EMA`)。
    - 触发公式: `((MACD > 0 AND close x> 5m EMA) OR (close > 5m EMA AND MACD x> 0)) AND (close > 1h EMA) AND (close < 日线 EMA)`
...
> **对策**：**坚决不做！**
> **理由**：
> *   **重复性**：**“横盘突破”的成功案例，往往与策略三 (Momentum) 高度重合**。如果真有大突破，MACD 动能必然会爆发，用 Momentum 策略抓更稳。
> *   **工具缺失**：没找到能干净过滤横盘的指标，没必要为了不确定性高的“左侧突破”去冒险，不如等右侧 MACD 确认。
