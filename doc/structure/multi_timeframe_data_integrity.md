...
本文档描述当前多周期映射机制，以及各模块如何避免未来数据泄露（Look-Ahead Bias）。
...
> [!IMPORTANT]
> **`DataPack.base_data_key` 必须对应所有 source 中的最小周期。**
...
1. `build_time_mapping`：数据进入 mapping 基础设施时校验并冻结 pack 合法性
2. 正式执行链路不再复制第二套入口 guard；pack 真值由 producer 入口收口
...
- 读取 `data_pack.source[key]` 的 OHLCV 数据用于 K 线图和成交量图
...
1. 测试路径固定：先 `ExecutionStage=Signals(TestPack)` 拿信号，再注入，再 `Backtest(TestPack)`，最后对正式测试结果做绩效分析。
2. 跨窗继承注入点固定在 `test active` 首根。
3. 强制离场注入点固定在测试包倒数第二根，以保证最后一根完成平仓。
4. `BorrowFromTrain` 与 `ExtendTest` 都要求窗口几何连续；两者差异只在测试预热段是否借用训练 active 尾部。
...
| 全部 source 时间 | `assert_source_times_strictly_increasing` | 严格递增 | 确保 stitched 后任一 source 都不存在重复时间或回退 |

**补充说明**：当前正式口径不再接受“非 base source 非递减即可”的工程兜底写法；base 与非 base source 统一按严格递增验收。
...
1. 本文档只维护“映射与完整性”主线。
2. WF 窗口模式、注入细节与 source time 正式约束以当前任务文档为准：
   - `doc/tasks/2026-03-10 unified ranges warmup wf/02_spec/`
   - `doc/tasks/2026-04-05 wf export display and precheck cleanup/02_spec/07_source_time_and_renko_contracts.md`
