...
fn process_single_df(df: &DataFrame) -> PolarsResult<DataFrame> {
    ...
    let result = pool.install(|| -> PolarsResult<DataFrame> {
        ...
        println!("[{}] Inside single-thread pool: Rayon threads = {}",
                 current_thread_id,
...
pub fn process_dataframes_vec(pydfs: Vec<PyDataFrame>) -> PyResult<Vec<PyDataFrame>> {
    ...
    let polars_env_value = env::var("POLARS_MAX_THREADS").unwrap_or_else(|_| "NOT_FOUND".to_string());

    println!("Python设置的环境变量 [TEST_ENV_KEY] 的值是: {}", test_env_value);
    println!("Python设置的环境变量 [polars_env_value] 的值是: {}", polars_env_value);
    ...
    println!("current_pool_size_before_op= {}", current_pool_size_before_op); // 再次打印

    // 触发 Polars POOL 初始化的简单操作示例（使用一个不会崩溃的表达式）
    // 理论上，任何 Polars 数据结构的使用都会触发
    let _s = Series::new("initializer".into(), &[1.0, 2.0, 3.0]);

    println!("强制初始化后（如果之前未初始化），Pool 大小: {}", rayon::current_num_threads());
    // ----------------------------------------------------

    println!("Outside par_iter: Current threads = {}", rayon::current_num_threads()); // 再次打印

    // 转换为 Vec<DataFrame>
    let dfs: Vec<DataFrame> = pydfs
        .into_iter()
        .map(|pydf| pydf.into()  )
        .collect();
